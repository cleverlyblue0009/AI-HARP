...
def run_task(task: dict[str, Any], checkpoint: str | None = None,
             duration_s: float | None = None, deterministic: bool = False,
...
def _run_star(args: tuple[dict[str, Any], str | None, float | None, bool, str]) -> dict[str, Any]:
...
def sweep(out_csv: Path, tasks: list[dict[str, Any]], checkpoint: str | None, jobs: int,
          ...
          deterministic: bool = False, backend: str = "auto") -> int:
    ...
    t0 = time.time()
    work = [(t, checkpoint, duration_s, deterministic, backend) for t in todo]
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--backend", default="auto", choices=["auto", "sumo", "fallback"],
                    help="mobility backend; 'auto' takes SUMO when SUMO_HOME is set")
    ...
    if args.backend == "sumo" and Path(args.out).name == "runs.csv":
        raise SystemExit("--backend sumo must write elsewhere (e.g. --out "
                         "results/runs_sparse_sumo.csv): runs.csv is the fallback-backend "
                         "evidence base and its rows carry no backend column")
